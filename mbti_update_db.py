# 로컬 서버 띄우기 test_완료됨 (변화량 기준 축 하나만 이동하는거)
# "나는 친구들을 만나는게 좋아" -> 45/50/50/50
...
axis_prompt_templates = {
    axis: PromptTemplate(
        ...
        성향 판단 방법:
        - '{left}' 성향에 더 가깝다고 판단되면 점수를 **하락**시켜. (점수 감소)
        ...
        "S": ("감각(S)", "직관(N)", "오감에 의존해 현재 사실과 디테일을 중시, 가능성과 비유를 통해 큰 그림을 보고 상상력을 발휘"),
        "T": ("사고(T)", "감정(F)", "객관적 사실과 논리에 기반해 판단, 정서와 인간관계를 중시해 판단"),
        "J": ("판단(J)", "인식(P)", "계획적이고 체계적으로 활동하며 빠른 결정을 선호, 유연하게 상황에 맞춰 즉흥적으로 행동하며 변화와 호기심 추구")
        }.items()
}
